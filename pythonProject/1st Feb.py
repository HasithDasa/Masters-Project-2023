...
with open('training_review.json') as f:
    data = json.load(f)
    data1 = data['images'][12]  # selecting the image
    ...
    # drawn_img_unit_7 = cv2.morphologyEx(drawn_img_unit_7, cv2.MORPH_CLOSE, kernel)
    ...
    # drawn_img_unit_7 = cv2.morphologyEx(drawn_img_unit_7, cv2.MORPH_CLOSE, kernel)
    # drawn_img_unit_7 = cv2.dilate(drawn_img_unit_7, kernel, iterations=1)
    ...
    # substracted_img = cv2.bilateralFilter(substracted_img, 9, 75, 75)



    # gray_subs = cv2.cvtColor(substracted_img, cv2.COLOR_BGR2GRAY)
    # img_for_cv_temp_7_Gray = cv2.cvtColor(img_for_cv_temp_7, cv2.COLOR_BGR2GRAY)


    # final_sub_img = img_for_cv_temp_7 - gray_subs

    # substracted_img = cv2.erode(substracted_img, kernel, iterations=2)

    # substracted_img = cv2.cvtColor(substracted_img, cv2.COLOR_BGR2GRAY)

    # binary_image_line_unit_RGB_7 = cv2.cvtColor(binary_image_line_unit_7, cv2.COLOR_GRAY2BGR)


    # Save the image to a file
    cv2.imwrite("leaves_plant.png", large_contours_img)

    # img_temp_lf_stm = result.astype(np.uint8) * 255

    # cv2.imwrite("img_temp_lf_stm.jpg", img_for_cv_temp_6)
...
#
#     # get contour
#     contours = cv2.findContours(img_temp_6, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
#     # result = img_temp_6.copy()
#     cv2.ellipse(img_for_cv_temp_7, ellipse, (0, 0, 255), 1)
#
#     # save results
#     # cv2.imwrite('leaf_threshold.png', thresh)


    # cv2.imwrite("img_temp_lf_stm.jpg", img_for_cv_temp_7)
        ...
        if clustering_needed:
            ...
            if len(clustering_len_list) > 2: #Check if clustering_len_list has more than two elements
                ...
                if clustering_len_list[min_index_clus] in [1, 2]: # Check if the minimum element is 1 or 2
                    ...
                    # index of the next smallest element even if the two elements are same or neighbouring element
                    ...
                    for next_min_clus_ele in next_min_clus_list:
                        min_clus_len_index = clustering_len_list.index(next_min_clus_ele)
                        ...
                        for cluster_coor_list_elem in cluster_coor_list:
                            # Check if the pixel is inside the contour
                            result = cv2.pointPolygonTest(contours[0], cluster_coor_list_elem, measureDist=False)
