...
with open('training_review.json') as f:
    data = json.load(f)
    data1 = data['images'][9]  # selecting the image
...
# considering as whole image
    ...
    for j_3 in range(len(list_of_keys)):
        ...
        if j_3 <= len(list_of_keys) - 2:
            selected_list_3[row_start_last:row_end_last, col_start_last:col_end_last] = 0
    ...
    # print("coordi_ones", co_with_ones[5][0][0])
...
# checking which mask belongs to which Bbox
    ...
    bbox_ones_mask_list1 = np.sort(np.array(bbox_ones_mask_list))
    bbox_ones_mask_list1 = np.unique(bbox_ones_mask_list1, axis=0)
    # bbox_ones_mask_list1 = set(bbox_ones_mask_list)
    with open("bbox_ones_mask_list.txt", "w") as output1:
        output1.write(str(bbox_ones_mask_list1))

    with open("bbox_list.txt", "w") as output1:
        output1.write(str(bbox_list))
