...
# checking which mask belongs to which Bbox
    ...
    for save_comm in intersected_comm_ones_list:
        ...
        if save_comm[1] == 7 and save_comm[2] == 11: # common parts in 2nd image (in 1st image)
            save_comm_list_1.append(save_comm[0])

        if save_comm[1] == 11 and save_comm[2] == 7: # common parts in 2nd image (in 1st image)
...
## canny try
    ...
    # (mu, sigma) = cv2.meanStdDev(and_edges_t4_t5)
    # edges = cv2.Canny(and_edges_t4_t5, mu - sigma, mu + sigma)
    # lines = cv2.HoughLines(and_edges_t4_t5, 1, np.pi/180, 70)
    ...
    # when length cluster index is 1st one, last one or any other one
        ...
        if min_clus_len_index == 0:
            ...
            min_clus_index = [clustering_list[len(clustering_list)-1], len(row_indexes)-1]
        else:
    ...
    resized_image_ori_rgb = cv2.cvtColor(img_for_cv, cv2.COLOR_GRAY2RGB)

    # drawPolyline
    cv2.polylines(resized_image_ori_rgb, [cluster_coor_list], isClosed_cl, color_cl, thickness_cl)
    ...
    resized_image_ori_rgb = cv2.resize(resized_image_ori_rgb, dim, interpolation=cv2.INTER_AREA)
