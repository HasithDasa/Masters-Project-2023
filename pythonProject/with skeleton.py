...
with open('training_done.json') as f:
    ...
    img_temp_shape = (data1['height'], data1['width'])
...
def opencv_format():
    ...
    return img_for_cv, bbox_list, lg_bbox_list, mask_list, co_with_ones, list_of_keys, dic_for_selection

# function => acquiring Bounding Boxes (def acq_bbox)

def acq_bbox(img_for_cv, bbox_list, lg_bbox_list, co_with_ones):
    ...
    return intersected_masks_list_final, intersected_comm_ones_list

# function => releasing image unit (def rel_img_unit)

def rel_img_unit(intersected_masks_list_final, intersected_comm_ones_list):
...
#going on one by one on intersected masks
    ...
    return list_of_keys_t5, dic_for_selection_t5, dic_for_selection_t4


# function => seperation of line segments from the image(def acq_line_seg)

def acq_line_seg(list_of_keys_t5, dic_for_selection_t5, dic_for_selection_t4):
    ...
    return and_edges_t4_t5, row_indexes, col_indexes, and_list, class_name_from_list_of_keys_t5, list_of_keys_t5_ind

# function => checking for clustering (def check_clust)

def check_clust(row_indexes, col_indexes):
    ...
    return clustering_list, clustering_list_col, clustering_list_row

# function => checking the which clustering it belongs to (def wh_clust)

def wh_clust(clustering_list, clustering_list_col, clustering_list_row, row_indexes, col_indexes, and_edges_t4_t5, class_name_from_list_of_keys_t5, intersected_masks_list_final, list_of_keys_t5_ind, list_of_keys, dic_for_selection):
...
if __name__ == "__main__":
    ...
    img_for_cv, bbox_list, lg_bbox_list, mask_list, co_with_ones, list_of_keys, dic_for_selection = opencv_format()
    intersected_masks_list_final, intersected_comm_ones_list = acq_bbox(img_for_cv, bbox_list, lg_bbox_list, co_with_ones)
