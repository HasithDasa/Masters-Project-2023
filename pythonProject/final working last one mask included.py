...
with open('training_review.json') as f:
    ...
    class_name_from_list_6 = list_of_keys[5]
...
# checking which mask belongs to which Bbox
    ...
    for list_of_keys_t5_ind in range(len(list_of_keys_t5)):
        ...
        for row_ind, row_val in enumerate(row_indexes):
            if row_ind < (len(row_indexes)-1):
                if abs(row_val-row_indexes[row_ind+1]) > 3: # checking the distance between two neighbouring pixels are higher than 5
        ...
        if clustering_needed:
            ...
            # checking the min index of the clustering len index, please add the code based on the length value, if length ==1 then ignore, then go to the next length
            min_clus_len_index = np.argmin(clustering_len_list)

            sorted_clustering_len_list = clustering_len_list.sort()
    ...
    for final_indx, final_elem in enumerate(final_completed_image_list):
        ...
        intersected_masks_name = "final_images/Intersected_mask_%d.jpg" % final_indx
        print(intersected_masks_name)
        cv2.imwrite(intersected_masks_name, final_elem)

    for and_indx, and_elem in enumerate(and_list):

        intersected_line_masks_name = "final_images_lines/Intersected_line_mask_%d.jpg" % and_indx
        # print(intersected_masks_name)
