...
# checking which mask belongs to which Bbox
    ...
    for ele_ind, ele_int_list in enumerate(intersected_masks_list):
        ...
        with open("save_comm_list_1.txt", "w") as output1x:
            output1x.write(str(save_comm_list_1))
        ...
        dic_for_selection_t5[key_t5] = img_for_cv_temp_5
        dic_for_selection_t4[key_t5] = img_for_cv_temp_4
        ...
        for list_of_keys_t5_ind in range(len(list_of_keys_t5)):
            ...
            if clustering_needed:
                ...
                clustering_len_list = [clustering_list[0] + 1]
                for index_clus in range(len(clustering_list)):
                    if index_clus > 0:
            ...
            # when length cluster index is 1st one, last one or any other one
                min_clus_index = []
                if min_clus_len_index == 0:
                    min_clus_index = [0, clustering_list[0]+1]
                elif min_clus_len_index == len(clustering_len_list) - 1:
                    min_clus_index = [clustering_list[len(clustering_list)-1]+1, len(row_indexes)-1]
                else:
                    min_clus_index = [clustering_list[min_clus_len_index -1]+1, clustering_list[min_clus_len_index]-1]
            ...
            final_masks_collection.append(saving_image_ori_rgb)
            final_coord_list.append(cluster_coor_list)
            ...
            resized_saving_image_ori_rgb = cv2.resize(saving_image_ori_rgb, dim, interpolation=cv2.INTER_AREA)

        intersected_masks_name = "final_images/Intersected_mask_%d.jpg" % ele_ind
        print(intersected_masks_name)
        cv2.imwrite(intersected_masks_name, resized_saving_image_ori_rgb)



    # for final_mask_indx, final_mask_elem in enumerate(final_masks_collection):
    #
    #     # color, thickness and isClosed
    #     color_cl = (255, 0, 0)
    #     thickness_cl = 2
    ...
    #
    #     # drawPolyline
    #     cv2.polylines(final_mask_elem, [final_coord_list[final_mask_indx]], isClosed_cl, color_cl,
    #                                          thickness_cl)
    #
    #     scale_percent = 50  # percent of original size
    #     width = int(img_width * scale_percent / 100)
    ...
    #     dim = (width, height)
    #
    #     resized_saving_image_ori_rgb = cv2.resize(final_mask_elem, dim, interpolation=cv2.INTER_AREA)
    #
    #     intersected_masks_name = "final_images/Intersected_mask_%d.jpg" % final_mask_indx
    #     print(intersected_masks_name)
    #     cv2.imwrite(intersected_masks_name, final_mask_elem)
    ...
    class_name_from_list_6 = list_of_keys[9]
