            ...
            if len(clustering_len_list) > 2: #Check if clustering_len_list has more than two elements
                ...
                if clustering_len_list[min_index_clus] in [1, 2]: # Check if the minimum element is 1 or 2
                    ...
                    if duplicates:
                        ...
                        contours_check, hii = cv2.findContours(and_edges_t4_t5, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

                        # Process only the largest contour whose length is less than 20
                        max_contour_length = 0
                        max_contour = None
                        for contour_elem in contours_check:
                            contour_length = cv2.arcLength(contour_elem, False)
                            print("contour_length", contour_length)

                            if contour_length < 5 and contour_length > max_contour_length:
                                max_contour_length = contour_length
                                max_contour = contour_elem
                        print("max_contour_length", max_contour_length)

                        # If a contour was found, draw it using cv2.polylines
                        if max_contour is not None:
                            x_max_contour, y_max_contour = max_contour[:, :, 0], max_contour[:, :, 1]
                            pts_12 = np.column_stack((x_max_contour, y_max_contour))
                            print("pts_12", pts_12)
                            and_edges_t4_t5_2 = and_edges_t4_t5 * 0
                            cv2.cvtColor(and_edges_t4_t5_2, cv2.COLOR_GRAY2BGR)

                            cv2.polylines(and_edges_t4_t5_2, [pts_12], True, (255, 255, 255), thickness=2)
                            cv2.imwrite("and_edges_t4_t5_2.png", and_edges_t4_t5_2)
                        ...
                        class_name_from_list_8 = list_of_keys[ele_int_list[0]]  # change unit
